...
\end{lstlisting}
\subsection{Trigger per la rimozione del numero di citazioni}
...
$$;
\end{lstlisting}
\subsubsection*{Trigger per l'update delle citazioni}
Il trigger controlla che una pubblicazione non si citi da sola e gestisce l'attributo derivato \textbf{NumeroCitazioni}
\begin{lstlisting}
...
CREATE or REPLACE FUNCTION updt_citazioni()
	RETURNS trigger LANGUAGE plpgsql AS $$
	BEGIN
		RAISE EXCEPTION 'Una citazione non puo' essere aggiornata si consiglia di aggiungerne una nuova ed eliminare la vecchia';
	END;
$$;
...
1. Controlla che il codice della pubblicazione associato alla tesi non sia già
utilizzato in altre pubblicazioni (articoli o libri), prevenendo così duplicazioni.
...
\begin{lstlisting}
    --3)Check dell'uni come affiliazione
--Prima inserimento effettivo TESI, controllo che l'affiliazione selezionata sia un'università
